...
export class FetchClientResolver {
  ...
  public json<V = unknown, R = V>(callback?: (value: V) => R): Promise<FetchClientError | Error | V | R> {
  ...
  public res<R = Response>(callback?: (value: Response) => R): Promise<FetchClientError | Error | Response | R> {
    if (!this._responsePromise) throw Error(MESSAGE_FETCH_CLIENT_ERROR);
  ...
  private async catchWrapper<T>(promise: Promise<T>): Promise<T | FetchClientError | Error> {
    try {
